/**
 * Latati Edge twin — Cloudflare Worker + KV (always on)
 * Parallel to Render; does not replace it.
 */
const CORS = {
...
function defaultStore(tenant) {
  ...
  return {
    profile: {
      ...
      address: "",
      bio: isLatati ? "La Tati · catálogo y pedidos (edge)" : "Catálogo y pedidos",
...
async function loadStore(env, tenant) {
  const key = `latati:v1:t:${tenant}:store`;
  const raw = await env.LATATI_KV.get(key, "text");
  if (!raw) {
    const st = defaultStore(tenant);
    await env.LATATI_KV.put(key, JSON.stringify(st));
    return st;
  }
...
async function saveStore(env, tenant, st) {
  st.rev = (st.rev || 0) + 1;
  await env.LATATI_KV.put(`latati:v1:t:${tenant}:store`, JSON.stringify(st));
  return st.rev;
}
...
export default {
  async fetch(request, env) {
    ...
    if (path === "" || path === "/") {
      return json({
        ok: true, name: "La Tati edge (Cloudflare KV)", twin: true,
        endpoints: ["catalog", "profile", "order", "orders", "chat", "gestor/login"],
      });
    }
    if (path === "sw.js") {
    ...
    if (path === "manifest.webmanifest" || path === "manifest.json") {
      return json({
        name: "La Tati Edge", short_name: "La Tati",
        start_url: "./index.html", display: "standalone",
        theme_color: "#e8b4c8", background_color: "#0c0a0f",
    ...
    const parts = parts0;
    const st = await loadStore(env, tenant);
    ...
    try {
      ...
      if (parts[0] === "gestor" && parts[1] === "profile" && request.method === "POST") {
        ...
        const rev = await saveStore(env, tenant, st);
      ...
      if (parts[0] === "gestor" && parts[1] === "products") {
        ...
        if (request.method === "POST" && parts.length === 2) {
          ...
          let p = st.products[id] || { id, created_at: new Date().toISOString(), unlimited: true, stock: 0, sold: false, sold_out: false, photo: false };
      ...
      if (parts[0] === "events") {
        ...
        const stream = new ReadableStream({
          async start(controller) {
            controller.enqueue(enc.encode(`event: hello\ndata: ${JSON.stringify({ ok: true, twin: true, rev: st.rev })}\n\n`));
            for (let i = 0; i < 20; i++) {
              await new Promise((r) => setTimeout(r, 2500));
              const cur = await loadStore(env, tenant);
